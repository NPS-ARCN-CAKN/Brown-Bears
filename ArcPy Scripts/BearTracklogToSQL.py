# BearTracklogToSQL.py
# Purpose: A Python script used by the National Park Service Arctic Network
# brown bear monitoring program.  Exports pilot's or observer's tracklog collected during aerial surveys
# to SQL insert queries suitable for importing the data into the program's master monitoring database.

# This Python script loops through the records in the submitted tracklog shapefile and writes an SQL insert query
...
Tracklogfile = arcpy.GetParameterAsText(0) #r'C:\Work\VitalSigns\ARCN Brown Bears\Data\2015_BearSurvey_SewardPen-BELA\Deliverable BB-02-Tracklogs\N527CC_22May2015_Tracklog.shp' # Source of the GPS Tracklog
...
# begine the sql script by writing some metadata
file.write("-- Insert queries to transfer pilot Tracklog to ARCN Bear monitoring database\n")
...
file.write("\n-- insert the generated tracklogs from " + Tracklogfile + " -----------------------------------------------------------\n")
...
arr = arcpy.da.FeatureClassToNumPyArray(Tracklogfile, ('ident', 'ltime','altitude','model','temp','comment','Latitude', 'Longitude'))
...
for row in arr:
    ...
    GPSModel = row["model"]
    PointFileName = str(os.path.basename(Tracklogfile))
    Notes = row["comment"]
    Lat = row["Latitude"]
    Lon = row["Longitude"]
